...
const createOrder = async (req,res)=>{
    try{
        ...
        if(!items || items.length === 0 || !totalAmount || !address){
            ...
            await order.save();
            const message = `Dear ${req.user.name},\\Thank you for your order! Your order has been successfully created with the following details.\n\norder ID:${order._id}\nTotal Amount:${totalAmount}\nshipping Address: ${address}\n\nWe will notify you once your order is shipped.\n\nBest regards.\nShopNest Team`;
...
const updateOrderStatus = async (req,res)=>{
    try {
        ...
        rse.status(500).json({message:'Error updating order status',error});
    }
}
